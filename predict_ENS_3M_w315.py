...
def Predict(test_all_features_np3D, input_file):
    ...
    out2 = Conv2D(filters=48, kernel_size=5, data_format="channels_last",
    ...
    model = keras.models.Model(inputs=[input_features,input_features2], outputs=out3)
    model.load_weights("models/CNN_T5_MSA_without315.h5") 
    ...
    while True:
        ...
        if not line_Pseq:
            break
        fout = open("Out_ENS_3M_w315_315/"+line_PID.upper()+".txt", "w")
