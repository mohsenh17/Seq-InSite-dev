...
class DataGenerator(keras.utils.Sequence):
    ...
    def readFeatures2D(self, neighborList,proteinName, protDict):
        selectedFeature = []
    ...
    def build2DWindows(self, aaIndex, pLength, proteinName, protDict, windowSize=WINDOW_SIZE):
        Features2D = []
        ...
        for i in range(100):
            if cnt < GLOBAL_NEIGHBOR_SIZE + addToRight + addToLeft:
                ...
                break
        Features2D = np.array(self.readFeatures2D(neighborList, proteinName,protDict))   
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        for i,item in enumerate(list_IDs_temp):
            ...
            if len(protDictChunk[protName]) == 0:
                protDict = self.protToDict(protName)
                protDictChunk.update(protDict)
            Features2D = self.build2DWindows(int(protAAIndex), int(protLength), protName, protDictChunk[protName])
            windows2D = Features2D
            X.append(np.array(windows2D))  
        ...
        if self.shuffleSet == True:    
            ...
            return np.array(XShuffled), np.array(yShuffled)
        else:
...
model = Sequential() 
input_features = Input(shape=((int)(WINDOW_SIZE), 1024), name="input_T5_1")
out3 = Dense(512, activation='relu', name="dense_T5_0")(input_features)
out3 = Dropout(rate=0.3)(out3)
out3 = Bidirectional(
        ...
        name="bidirectional_T5")(out3)
out3 = Dropout(rate=0.3)(out3)
out3 = Flatten()(out3)
out3 = Dense(64, activation='relu', name="dense_LSTM_T5_1")(out3)
out3 = Dropout(rate=0.3)(out3)
out3 = Dense(1, activation='sigmoid', name="dense_LSTM_T5_2")(out3)
model = keras.models.Model(inputs=input_features, outputs=out3)
"""
model = Sequential() 
...
mc = ModelCheckpoint("models/LSTM_t5U50_L9_5L.h5",
